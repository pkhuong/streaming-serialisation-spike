...
int main()
{
    ...
    {
        producer.write(std::span(reinterpret_cast<const std::byte *>(data.data()), data.size()));
    };

    {
        producer.push_substream();

        producer.varlen_field(1);
        write("12");
        producer.close_varlen_field();

        producer.varlen_field(8);
        {
            {
                producer.push_substream();
                producer.varlen_field(2);
                write("abcde");
                producer.close_varlen_field();

                producer.varlen_field(10);
                write("hjk");
                producer.close_varlen_field();

                producer.flush_substream();

                producer.varlen_field(2);
                write("zsxcvbnm,./0123456789a");  // Force a final full-sized chunk with test constants
                producer.close_varlen_field();

                producer.pop_substream();
            }

            producer.separate_substream();

            {
                producer.push_substream();
                producer.varlen_field(2, /*force_last=*/true);
                write("asdf");
                producer.close_varlen_field();
                producer.pop_substream();
            }
